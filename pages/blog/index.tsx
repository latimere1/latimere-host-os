...
export const getStaticProps: GetStaticProps<Props> = async () => {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') console.info('[blog/index.getStaticProps] slugs:', posts.map(p => p.slug))
    return { props: { posts } }
  } catch (err) {
...
export default function BlogIndex({ posts }: Props) {
  ...
  useEffect(() => { if (typeof window !== 'undefined') setClientOrigin(window.location.origin) }, [])

  const canonicalUrl = useMemo(() => {
    if (clientOrigin) return `${clientOrigin}/blog`
    if (appUrlEnv) return `${appUrlEnv}/blog`
    return '/blog'
  }, [clientOrigin, appUrlEnv])
  ...
  return (
    <>
      ...
      <section className="relative border-b border-white/10 bg-gray-950 text-white">
        <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(ellipse_at_top,_rgba(0,120,120,0.35),transparent_65%)]" />
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          <div className="grid items-center gap-8 lg:grid-cols-2">
            <div>
              <h1 className="text-4xl font-semibold tracking-tight sm:text-5xl">Insights that turn STRs into standouts.</h1>
              ...
              <div className="mt-6 flex gap-3">
                ...
                <a href="/#services" className="rounded-lg border border-white/15 bg-white/5 px-4 py-2.5 text-sm font-medium hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-cyan-400/50">
                  See what’s included
                </a>
              </div>
        ...
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          {posts.length === 0 ? (
            ...
            <ul className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {posts.map((p) => (
                <li key={p.slug} className="group rounded-2xl border border-white/10 bg-white/[0.04] p-3 transition hover:bg-white/[0.06]">
                  <Link
                    href={`/blog/${p.slug}`}
                  ...
                  >
                    ...
                    <div className="px-1 pt-3">
                      <div className="text-[12px] text-gray-400">{p.date} · {p.author || 'Latimere Team'}</div>
          ...
          <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 text-white lg:flex lg:items-center lg:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-1 text-sm text-gray-300">Get a free revenue estimate and a concrete 30-day plan to improve occupancy and ADR.</p>
              </div>
              <div className="mt-4 flex gap-3 lg:mt-0">
                <Link href="/#contact" className="rounded-lg bg-cyan-500 px-4 py-2.5 text-sm font-semibold text-gray-900 hover:bg-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/70" onClick={() => console.info('[CTA] blog footer → Estimate')}>
                  Get my estimate
                </Link>
                <a href="/#services" className="rounded-lg border border-white/15 bg-white/5 px-4 py-2.5 text-sm font-medium hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-cyan-400/50" onClick={() => console.info('[CTA] blog footer → Services')}>
                  See services
                </a>
              </div>
            </div>
